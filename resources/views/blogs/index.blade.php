<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
        ...
        <a href="{{ route('blogs.show', $blog) }}" class="card group hover:shadow-elevated transition-all duration-300">
            @if($blog->cover_image)
            <img src="{{ asset('storage/' . $blog->cover_image) }}" alt="" class="w-full h-40 object-cover rounded-xl mb-4">
            @else
            ...
            @endif
            <h3 class="font-semibold text-content group-hover:text-primary transition-colors line-clamp-2">{{ $blog->title }}</h3>
            <p class="text-sm text-content-muted mt-2 line-clamp-2">{{ $blog->excerpt ?? Str::limit(strip_tags($blog->content), 120) }}</p>
            <div class="flex items-center gap-2 mt-4 pt-4 border-t border-surface-border">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-content-muted">{{ $blog->published_at?->diffForHumans() }}</p>
                </div>
            </div>
        </a>
        @empty
